...
public class Automaton {
	// Class constants:
	public static final String EPSILON = "" + (char) 163, NOSTATE = "-",
			PRIMARY_NAME = "EE\\TT";
	protected static final MyTableRenderer tableRenderer = new MyTableRenderer(
			MyTableRenderer.AUT);
	...
	public static String add(String s, int bound) {
		...
		if (s == NOSTATE)
			return s;
		if (s.charAt(0) == '{') {
			// Obtain a String with only numbers considering the format: "{1, 2,
			// ..., n}."
			s = s.replace('{', ' ');
			s = s.replace('}', ' ');
			s = s.replace(',', ' ');
	...
	public static Automaton importFromFile(String path) throws FormatException {
		FileReader fr = null;
	...
	public static String[][] newTable(int y, Object[] headers) {
		...
		for (int i = 0; i < table.length; i++)
			for (int j = 0; j < table[0].length; j++) {
				if (i == 0 && j > 0)// The first row contains the name of the
									// table headers.
					table[0][j] = headers[j - 1].toString();
				else if (i == 0 && j == 0)// The first cell always will be
											// EE//TT.
					table[0][0] = PRIMARY_NAME;
				else if (j == 0 && i > 0)// The first column if row>0 would be
											// the states name.
					table[i][0] = "" + i;
				else
					table[i][j] = NOSTATE;
	...
	// Operations:
	// Constructors:
	// Main constructor, it build an Automaton by creating two states with a
	// transition s.
	public Automaton() {
	...
	public Automaton(char type) {
		...
		switch (type) {
			case 'l' :
				...
				headers[52] = EPSILON;
				transitionTable = newTable(3, headers);
	...
	public Automaton concatenation(Automaton a) {
		...
		// This set help us to know how many transitions will have the next
		// Automaton.
		...
		String[][] table = newTable(
				a.transitionTable.length + this.transitionTable.length - 2,
				set.toArray());
		// bound contains the number of states of the first table in order to
		// add that quantity to the next table.
		...
		for (int i = 1; i < table.length; i++)
			for (int j = 1; j < table[0].length; j++)
				if (i < bound)
					...
					// getNextState returns a String with the next state with a
					// given transition.
					table[i][j] = this.getNextState(i, table[0][j]);
				else
					// i>=bound means we are on the second table, so we use add.
					// The add method edits the states to make them fit with the
					// states of the first Automaton.
					table[i][j] = add(
							a.getNextState(i - bound + 1, table[0][j]),
							bound - 1);
	...
	public void exportToFile(String fileName) {
		try {
			...
			writer.println(
					transitionTable.length + " " + transitionTable[0].length);
			for (int i = 0; i < transitionTable.length; i++)
				for (int j = 0; j < transitionTable[i].length; j++) {
					if (i == 1 && j == 0) {
						// Setting the initial state.
						line = "(inicial)" + transitionTable[i][j];
						if (finalStates.contains(i))
							line += "(final)";
					} else if (j == 0 && i > 0 && finalStates.contains(i))
						// Setting the final states.
						line = transitionTable[i][j] + "(final)";
					else
						// Just filling.
						line = transitionTable[i][j];
					writer.println(line);
				}
			writer.close();
		} catch (Exception e) {
	...
	public String getNextState(int state, String transition) {
		...
		for (j = 1; j < transitionTable[0].length; j++)
			if (transitionTable[0][j].equals(transition))
				break;
		// If v then there is no transition.
		if (j == transitionTable[0].length)
			return NOSTATE;
	...
	public JTable getTable() {
		...
		for (int i = 0; i < table.length; i++)
			for (int j = 0; j < table[i].length; j++)
				if (j == 0 && i == 1) {
					table[i][j] = "(inicial)" + transitionTable[i][j];
					if (finalStates.contains(i))
						table[i][j] += "(final)";
				} else if (j == 0 && i > 0 && finalStates.contains(i))
					table[i][j] = transitionTable[i][j] + "(final)";
				else
					table[i][j] = transitionTable[i][j];
		// The table style.
	...
	public Automaton optional() {
		String[][] table = newTable(this.transitionTable.length + 2,
				this.transitionTable[0]);
	...
	public Automaton plus() {
		...
		table[table.length - 2][table[0].length - 1] = "{ 2, "
				+ (table.length - 1) + "}";
	...
	// Useful methods.
	public int size() {
		return transitionTable.length - 1;
	}
	...
	public Automaton union(Automaton a) {
		...
		// Create an String array according with bounds: statesXset's size.
		String[][] table = newTable(
				this.transitionTable.length + a.transitionTable.length + 1,
				set.toArray());
		// bound contains the number of states of the first table in order to
		// add that quantity to the next table.
		int bound = 1;
		...
		for (int i = 1; i < this.transitionTable.length; i++)
			for (int j = 1; j < table[0].length; j++)
				table[i + bound][j] = add(this.getNextState(i, table[0][j]),
						bound);
		...
		for (int i = 1; i < a.transitionTable.length; i++)
			for (int j = 1; j < table[0].length; j++)
				table[i + bound][j] = add(a.getNextState(i, table[0][j]),
						bound);
		// Some union's characteristics.
		table[1][table[0].length - 1] = "{ 2, "
				+ (this.transitionTable.length + 1) + "}";
		table[table.length - 2][table[0].length - 1] = "" + (table.length - 1);
		table[this.transitionTable.length][table[0].length - 1] = ""
				+ (table.length - 1);

...
public class AFD extends Automaton {
	...
	// Operations:
	// Constructors:
	public AFD(String expression) throws FormatException {
	...
	public HelpingState clausureE(Automaton a, HelpingState clausure) {
		...
		while (!stack.isEmpty()) {
			t = stack.pop();
			// To get access one by one of the elements in Automaton transition
			// EPSILON.
	...
	public void powerSetAlgorithm(Automaton a) {
		...
		String[] headers;
		if (a.transitionTable[0][a.transitionTable[0].length - 1]
				.equals(EPSILON))
		...
		for (int i = 1; i < this.transitionTable.length; i++)
			for (int j = 1; j < this.transitionTable[i].length; j++)
				// The purpose of this is print the name of the state instead
				// the states in subSet.
				if ((next = statesD.elementAt(i - 1).getTransition(j)) != null)
					for (int k = 0; k < statesD.size(); k++) {
						if (statesD.elementAt(k).compareTo(next) == 0) {
							transitionTable[i][j] = statesD.elementAt(k)
									.realName();
							break;
						}
					}
	...
	public boolean processApart(String word) {
		int currentState = 1;
		String s;
		Files.printConsole(transitionTable);

		for (int i = 0; i < word.length(); i++) {
			s = getNextState(currentState, "" + word.charAt(i));
			if (s.equals(NOSTATE))
				return false;
			currentState = Integer.parseInt(s);
			System.out.print("s:" + s + " " + word.charAt(i) + "¿?");
		}
